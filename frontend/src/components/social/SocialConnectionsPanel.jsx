...
export function SocialConnectionsPanel({ channelType = 'CREATOR' }) {
  ...
  return <section className="social-connections-panel relative isolate rounded-2xl border border-white/10 bg-white/[.025] p-4 sm:p-5">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
      <div className="min-w-0"><strong className="block text-sm">Social channels</strong><small className="mt-1 block max-w-2xl text-[11px] leading-5 text-white/40">Connect a Facebook Page or Instagram professional account. Verified provider metrics remain separate from links you add manually.</small></div>
      <div className="grid w-full shrink-0 grid-cols-1 gap-2 min-[430px]:grid-cols-2 lg:w-auto">{['instagram', 'facebook'].map((provider) => <Button className="w-full whitespace-nowrap lg:w-auto" key={provider} type="button" size="sm" variant="outline" loading={busy === provider} onClick={() => connect(provider)}>Connect <span className="capitalize">{provider}</span></Button>)}</div>
    </div>
    {loading ? <div className="grid min-h-28 place-items-center"><Spinner label="Loading social channels" /></div> : accounts.length ? <div className="mt-4 grid gap-2">{accounts.map((account) => {
      ...
      return <article key={account.id} className="grid min-w-0 gap-3 rounded-xl border border-white/[.08] bg-black/10 p-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center lg:grid-cols-[minmax(0,1fr)_auto_auto_auto_auto_auto]">
    ...
    <div className="social-manual-form mt-4 grid gap-2 border-t border-white/[.07] pt-4 md:grid-cols-2 xl:grid-cols-[9rem_minmax(0,1fr)_minmax(10rem,.6fr)_auto]">
      <Select menuPlacement="top" aria-label="Manual social platform" value={manual.platform} onChange={(event) => setManual((value) => ({ ...value, platform: event.target.value }))} options={['YOUTUBE', 'TIKTOK', 'INSTAGRAM', 'FACEBOOK', 'X', 'OTHER']} />
      ...
      <Button className="w-full xl:w-auto" type="button" size="sm" variant="outline" loading={busy === 'manual'} onClick={addManual}><Plus size={13} />Add link</Button>
    </div>
